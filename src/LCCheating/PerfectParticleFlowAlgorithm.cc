...
PerfectParticleFlowAlgorithm::PerfectParticleFlowAlgorithm()
    : m_simpleCaloHitCollection(true), m_minWeightFraction(0.01f) {}
...
StatusCode PerfectParticleFlowAlgorithm::Run() {
  ...
  for (MCParticleList::const_iterator iterMC = pMCParticleList->begin(), iterMCEnd = pMCParticleList->end();
...
void PerfectParticleFlowAlgorithm::CaloHitCollection(const MCParticle* const pPfoTarget,
                                                     PfoParameters& pfoParameters) const {
  ...
  for (CaloHitList::const_iterator iter = localCaloHitList.begin(), iterEnd = localCaloHitList.end(); iter != iterEnd;
       ++iter) {
    try {
...
void PerfectParticleFlowAlgorithm::SimpleCaloHitCollection(const MCParticle* const pPfoTarget,
...
void PerfectParticleFlowAlgorithm::FullCaloHitCollection(const MCParticle* const pPfoTarget,
                                                         const CaloHit* const pCaloHit,
                                                         CaloHitList& caloHitList) const {
...
void PerfectParticleFlowAlgorithm::TrackCollection(const MCParticle* const pPfoTarget,
                                                   PfoParameters& pfoParameters) const {
...
void PerfectParticleFlowAlgorithm::SetPfoParametersFromTracks(const MCParticle* const pPfoTarget, int& nTracksUsed,
                                                              PfoParameters& pfoParameters) const {
  if (!pfoParameters.m_trackList.empty()) {
    ...
    for (TrackList::const_iterator iter = pfoParameters.m_trackList.begin(), iterEnd = pfoParameters.m_trackList.end();
         iter != iterEnd; ++iter) {
      const Track* const pTrack = *iter;

      if (!pTrack->CanFormPfo() && !pTrack->CanFormClusterlessPfo()) {
        std::cout << pPfoTarget << " Drop track, E: " << pTrack->GetEnergyAtDca() << " cfp: " << pTrack->CanFormPfo()
                  << " cfcp: " << pTrack->CanFormClusterlessPfo() << std::endl;
        continue;
      }

      if (!pTrack->GetParentList().empty()) {
        std::cout << pPfoTarget << " Drop track, E: " << pTrack->GetEnergyAtDca()
                  << " nParents: " << pTrack->GetParentList().size() << std::endl;
        continue;
      }
      ...
      energyWithElectronMass +=
          std::sqrt(electronMass * electronMass + pTrack->GetMomentumAtDca().GetMagnitudeSquared());
    }
    ...
    pfoParameters.m_particleId = (pfoParameters.m_charge.Get() == 0)  ? PHOTON
                                 : (pfoParameters.m_charge.Get() < 0) ? PI_MINUS
                                                                      : PI_PLUS;
    ...
    pfoParameters.m_mass =
        std::sqrt(std::max(pfoParameters.m_energy.Get() * pfoParameters.m_energy.Get() -
                               pfoParameters.m_momentum.Get().GetDotProduct(pfoParameters.m_momentum.Get()),
                           0.f));
  }
}
...
void PerfectParticleFlowAlgorithm::SetPfoParametersFromClusters(const MCParticle* const pPfoTarget,
  ...
  if ((0 == nTracksUsed) && !pfoParameters.m_clusterList.empty()) {
    ...
    const CartesianVector positionVector(!isPhoton ? pCluster->GetCentroid(pCluster->GetInnerPseudoLayer())
                                                   : this->GetEnergyWeightedCentroid(pCluster,
  ...
  if (NULL != pCluster) {
    for (TrackList::const_iterator iterTrk = pfoParameters.m_trackList.begin(),
                                   iterTrkEnd = pfoParameters.m_trackList.end();
         iterTrk != iterTrkEnd; ++iterTrk)
      PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS, !=,
                              PandoraContentApi::AddTrackClusterAssociation(*this, *iterTrk, pCluster));
  }
}

//------------------------------------------------------------------------------------------------------------------------------------------

const CartesianVector PerfectParticleFlowAlgorithm::GetEnergyWeightedCentroid(
    const Cluster* const pCluster, const unsigned int innerPseudoLayer, const unsigned int outerPseudoLayer) const {
  float energySum(0.f);
...
void PerfectParticleFlowAlgorithm::PfoParameterDebugInformation(const MCParticle* const pPfoTarget,
                                                                const int nTracksUsed,
                                                                PfoParameters& pfoParameters) const {
  if ((0 == nTracksUsed) && pfoParameters.m_clusterList.empty()) {
  ...
  if (std::fabs(pfoParameters.m_energy.Get() - pPfoTarget->GetEnergy()) > 2.f) {
    std::cout << pPfoTarget << " Did not match pfo target energy, target: " << pPfoTarget->GetParticleId()
              << ", E: " << pPfoTarget->GetEnergy() << " (reco: " << pfoParameters.m_particleId.Get()
              << ", E: " << pfoParameters.m_energy.Get() << ", nTrk: " << pfoParameters.m_trackList.size()
...
StatusCode PerfectParticleFlowAlgorithm::ReadSettings(const TiXmlHandle xmlHandle) {
  ...
  PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ReadValue(xmlHandle, "SimpleCaloHitCollection", m_simpleCaloHitCollection));

  PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=,
                                  XmlHelper::ReadValue(xmlHandle, "MinWeightFraction", m_minWeightFraction));
